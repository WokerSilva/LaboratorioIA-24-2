...
\end{lstlisting}
    \item Para Linux
\begin{lstlisting}[language=bash]    
    &\textcolor{rubber}{Skynet}& : &\textcolor{Meadow}{/src}& $ python_-m_venv_venv    
\end{lstlisting}
\end{itemize}
...
Necesitamos hacer las siguientes importaciones en el código para tener el funcionamiento de de python
con las aplicaciones web (para eso es flask), se necesita por supuesto la importación de 
chatterbot para construir el bot y trainers es utilizado para hacer el entrenamiento y hable 
de manera más fluida con el usuario. Finalmente json para trabajar con estos archivos pues son los 
que contienen toda la información de entrenamiento.

\begin{lstlisting}[style=mystylepython, language=Python, caption=Fragmento de código Python]    
...
Entonces trabajaremos con un chatbot, debemos crearlo y eso se hace así:

\begin{lstlisting}[style=mystylepython, language=Python, caption=Fragmento de código Python]    
    chatbot = ChatBot("ChatDog")
\end{lstlisting}

\textit{ChatDog} es el nombre con el cual distinguimos a nuestro bot y es la referencia para hacer
